...
This example dataset demonstrates a complete VQA application, including calculation of quality scores, quality hectares and net quality hectares (NPI) for a project with one offset. It presents a more complex analysis than vqa-demo1.
...
Ensure you have the following projects directory structure for this project:

```
projects/
 |__vqa-demo1/     [project directory]
     |__data/      [data directory]
         |__vqa-demo2/     [project data directory]
             |__offset_baseline/      [assessment directory]
             |__offset_current/       [assessment directory]
             |__project_baseline/     [assessment directory]
             |__project_current/      [assessment directory]
             |__raw/                  [raw data directory]
```

* Project directory `vqa-demo2` must be named exactly as shown.
* Project data directory `vqa-demo2 ` must be named exactly as shown.
...
```
Rscript --mode import --project vqa-demo2 --assess project_baseline 
```

### 2. Run VQA

```
Rscript vqa.R --mode vqa.batch --project vqa-demo2 --assess project_baseline 
```


### 3. Run Net Quality Hectares calculation
* Complete all VQA analyses for all assessments before running net quality hectares, as qh.net requires VQA results from multiple VQA assessments as inputs).

```
Rscript vqa.R --mode vqa.batch --project vqa-demo2 --assess main_001_current 
```

##### To include the offset in the QH.net analysis of the project site:
1. Run the QH.net assessment for the offset site (relative to the offset baseline)
2. Set the following parameter in `params.vqa-demo2.R`:

```
INCLUDE.OFFSET <- TRUE
```
* After starting qh.net, all key parameter values will be displayed. Verify that INCLUDE.OFFSET is set to TRUE before entering “y” to continue.
...
<div style="width:50px">Order</div> | <div style="width:50px">Main or offset</div> | <div style="width:90px">Assessment</div> | <div style="width:100px">Mode</div> | Notes
:------: | :------ | :------ | :------ |  :------
...
VQA supports a diversity of plot data types and formats. One purpose of this flexibility is to allow the use of legacy plot datasets, especially as benchmark data. However, the Excel spreadsheet format use in this demo (see raw data directory) is the more or less stable raw data format for new projects which are willing to adopt a “standard” VQA format. Note that the plot design and field sampling methods that produced the demo2 dataset was designed specifically for VQA.
...
Import (`--mode import`) generates the standard VQA input files in directory and saves them to directory `input/`. 

### VQA

The VQA pipeline (`--mode vqa.batch`) generates the files found in directory `results/`. The most important of these is `vqa_summary.R`, which provides a human-readable summary of all results, formatted as an Excel workbook. 
