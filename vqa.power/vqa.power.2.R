...
# Set the vegetation you wish to model
# FOR TESTING ONLY
...
# LIST OF LAND COVER CLASSES TO PROCESS
...
# Use this one for vqa-pub / reclamation
df.veg<-data.frame(veg=c(
"Brushland/Grassland (reclaimed)",
"Dry forest, Early-mid (reclaimed)",
"Intermediate forest, Early-mid (reclaimed)",
...
# Use this one for vqa-pub / baseline
df.veg<-data.frame(veg=c(
"Alpine",
"Alpine Dwarf Shrub",
"Avalanche feature",
...
sims <- 1000		# No of replicates per power test (at each sample size)
n.start <- 3				# Starting (minimum) sample size
...
sims.file <- paste0(RESULTSDIR, "power/power.sims.csv")

# Final results file
# Individual replicates (power calculations based on multiple
# replicates) are appended to this file
results.file <- paste0(RESULTSDIR, "power/power3.test.csv")

# Bootstrap results file
boot.file <- paste0(RESULTSDIR, "power/power3.boot.test.csv")
